...
class TennisMatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        diff = compute_player_differences(row)
        p1_experience = compute_experience(self.history,row["j1"],current_date)
        p2_experience = compute_experience(self.history,row["j2"],current_date)
        ...
        def one_hot_surface(surface):
            surfaces = ["DUR", "TERRE BATTUE", "DUR (INDOOR)", "GAZON"]
        ...
        def one_hot_tour(tour):
            tours = ["1/16 DE FINALE","DEMI-FINALES","QUARTS DE FINALE","1/8 DE FINALE",
                     "1/32 DE FINALE","1/64 DE FINALE","FINALE","3E PLACE"]
            
            vec = np.zeros(len(tours) + 1, dtype=np.float32)
            if tour in tours:
            ...
            vec[idx] = 1.0
            return vec
        tour_vec = one_hot_tour(row["tour"])
