def createStats():
    ...
    prev_stats["elo_players"] = defaultdict(lambda: 1500)
    prev_stats["elo_surface_players"] = defaultdict(lambda: defaultdict(lambda: 1500))
...
def getStats(player1, player2, match, prev_stats):
    ...
    output["TOURNEY_LEVEL"] = match["TOURNEY_LEVEL"]
    ...
    ####################### GET STATS ########################
    output["p1_elo"] = elo_players[PLAYER1_ID]
    output["p2_elo"] = elo_players[PLAYER2_ID]
